...
const targetFile = "C:\\Users\\OMPRAKASH KUMAR\\Projects\\green-pizzeria\\apps\\web\\app\\admin\\products\\page.tsx";
const backupFile = "C:\\Users\\OMPRAKASH KUMAR\\Projects\\green-pizzeria\\apps\\web\\admin-products-page-1.tsx.bak";

if (fs.existsSync(targetFile)) {
  fs.copyFileSync(targetFile, backupFile);
  console.log("Backed up existing file to: " + backupFile);
} else {
  console.log("WARNING: target file did not exist, nothing to back up.");
}
...
export default function AdminProductsPage() {
  ...
  const openNewForm = (presetCategoryId?: string) => {
    setEditingId(null);
    setForm({ ...EMPTY_FORM, categoryId: presetCategoryId || "" });
    setImageFile(null);
  ...
  const renderProductCard = (product: Product) => (
    <div key={product.id} className="border rounded-lg p-3 sm:p-4 mb-3 flex gap-3 sm:gap-4 overflow-hidden">
  ...
  return (
    <div className="max-w-3xl mx-auto px-3 sm:px-4 py-6 sm:py-10 overflow-x-hidden">
      {mode === "list" && (
        <>
          ...
          {productsByCategory.map(({ category, items }) => (
            <div key={category.id} className="mb-7 sm:mb-8">
              <div className="flex flex-wrap items-center justify-between gap-2 mb-3 pb-2 border-b border-dark/10">
                <h2 className="text-base font-semibold break-words">
                  {category.name}{" "}
                  <span className="text-sm font-normal text-muted">({items.length})</span>
                </h2>
                <button
                  onClick={() => openNewForm(category.id)}
      ...
      {mode === "form" && (
        <>
          <div className="flex flex-wrap items-center justify-between gap-3 mb-6 sm:mb-8">
            <h1 className="text-xl sm:text-2xl font-semibold">{editingId ? "Edit product" : "New product"}</h1>
...
console.log("Rewritten: " + targetFile);
